...
Inside a form nobody writes that call. `decide` (`app/forms/engine/decide.py`)
returns the events of each decision in `Decision.analytics`, and
`observability.emit` (`app/forms/adapters/discord/observability.py`) is the one
...
`_on_expired` in `decide.py`); the adapter decides *how* it is emitted, once,
...
setup.validation_failed:
  ...
  emitted_at: app/forms/adapters/discord/observability.py — emit, from Decision.analytics (Engine.refuse)
  actor: user
...
calls, and `app/forms/` never reaches them at all (`tests/forms/test_boundary.py`
...
`FormSession` (`app/forms/engine/session.py`) lives in the session store for
