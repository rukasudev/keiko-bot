...
- **Files:** new `app/settings/form.py`, `app/languages/form/*.yml`,
...
- **Change:** introduce `FormSession` and `SessionStore`; `Form` holds a `session_id`
  ...
  explicit parent id.
- **Files:** new `app/settings/session.py`, `store.py`; `form.py`, `form_state.py`
...
- **Files:** new `app/settings/form.py`, `events.py`, `effects.py`, `screen.py`,
...
- **Tooling** (`make lint`, CI): `ruff` (format + lint, including `D` docstring rules
  ...
  `mypy --strict` on `app/settings/`, and a custom check that fails on a `#` comment
...
- `app/settings/` is a package with **no import of `discord`** outside
  `app/settings/discord/`, enforced by a boundary test like
...
- **Style**: II.6 tooling in CI from the first commit of `app/settings/`.
...
**Phase C — Build `app/settings/` complete, in isolation (the bulk of the work).**
