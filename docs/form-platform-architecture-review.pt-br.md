...
- **Arquivos:** novo `app/forms/definitions.py`, `app/languages/form/*.yml`,
...
- **Arquivos:** novo `app/forms/session.py`, `store.py`; `form.py`, `form_state.py`
...
- **Mudança:** mover `_update_form_step`, `_should_skip_step`, `_save_step_response`,
  ...
  adapter.
- **Arquivos:** novo `app/forms/engine.py`, `events.py`, `effects.py`, `screen.py`,
...
- **Ferramentas** (`make lint`, CI): `ruff` (format + lint, incluindo regras `D` de
  ...
  `ARG`, `RET`), `mypy --strict` em `app/forms/`, e uma checagem própria que falha em
...
- `app/forms/` é um pacote **sem import de `discord`** fora de
  `app/forms/adapters/discord/`, garantido por um teste de fronteira como
...
- **Estilo**: ferramentas da II.6 no CI desde o primeiro commit de `app/forms/`.
...
**Fase C — Construir `app/forms/` completo, em isolamento (o grosso do trabalho).**
