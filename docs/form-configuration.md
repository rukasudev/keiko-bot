...
The map of the form platform (`app/forms/`): what a form declaration may say,
...
| Layer | Package | Owns |
|---|---|---|
| Definitions | `app/forms/definitions/` | the typed schema (`schema.py`), the YAML compiler with its legacy aliases (`compiler.py`), the registry and the source of definitions (`registry.py`, `source.py`), the JSON schema export |
| Engine | `app/forms/engine/` | the frozen session (`session.py`), events (`events.py`), effects (`effects.py`), the `when` evaluator (`rules.py`), the screen model (`screen.py`), `decide` (`decide.py`), the session store (`store.py`), documents and summaries (`documents.py`, `summary.py`), replay (`replay.py`) |
| Step kinds and extensions | `app/forms/kinds/`, `app/forms/extensions/` | one render and parse pair per kind; validators, transforms, formatters, copy resolution, links and dates |
| Features | `app/forms/features/` | one module per feature: answers to document and back, the commit, side actions |
| Discord adapter | `app/forms/adapters/discord/` | the entry points (`entrypoints.py`), the renderer (`renderer.py`), the executor (`executor.py`), the `custom_id` codec (`ids.py`), interaction to event (`interactions.py`), observability (`observability.py`) |

Only the adapter imports `discord`. Nothing in `app/forms/` names a command
...
1. A cog's slash command calls the feature service's `manager(interaction,
   ...
   (`app/forms/adapters/discord/entrypoints.py`). The `/setup` buttons reach
...
2. `Runtime.open_feature` asks the feature module (`feature_for(key)`,
   `app/forms/features/__init__.py`) to `open` the guild: it answers with the
...
3. The definition comes from the registry (`registry.get(key)`), which
   ...
   `compile_form` (`app/forms/definitions/compiler.py`). A definition that
...
4. Every interaction on a form message carries a `custom_id` of the shape
   `k:<session>:<revision>:<action>[:<arg>]` (`ids.py`). The adapter decodes
...
`app/forms/definitions/schema.py` is the whole vocabulary: pydantic models
...
`app/forms/engine/rules.py`; the compiler checks every rule before the form
exists.
...
|---|---|---|
| Step kinds | `registry()` in `app/forms/kinds/__init__.py`, one `Kind(render, parse)` per name | the eleven kinds of 3.2 |
| Validators | `VALIDATORS` in `app/forms/extensions/validators.py`; each declares `needs` (`answers`, `items`, `external:<service>`) so the adapter prefetches before `decide` | `validate_streamer_name`, `validate_youtube_channel`, `validate_link_or_domain`, `validate_date` |
| Transforms | `TRANSFORMS` in `app/forms/extensions/transforms.py`; a `Transform` names its `part_keys`, `value_key`, `style`, `serialize` and `hydrate` | `mm_dd_date_parts`, `normalize_link` |
| Formatters | `STYLES` and `format_value` in `app/forms/extensions/formatters.py` | the styles of 3.1 |
| Copy tokens | `TOKEN_FORMATTERS` in `app/forms/kinds/common.py` | `host` |
| Copy resolution | `text(key, locale)` and `normalize_locale` in `app/forms/extensions/copy.py` | reads the language files through `ml` |
| Definitions | `DefinitionSource` (`load`, `list`) in `app/forms/definitions/source.py` | `DiskSource` over `app/languages/form/` |
| Features | `feature_for(key)` in `app/forms/features/__init__.py` | the seven feature modules; any other compiled form gets `GenericCogFeature` |
...
`FeatureModule` (`app/forms/features/protocol.py`) is what a feature
...
feature's document. `GenericCogFeature` (`app/forms/features/generic.py`)
...
reader outside the forms depends on; `app/forms/engine/documents.py` is the
...
1. **The definition**, `app/languages/form/<key>.yml`: `steps:` composed
   ...
   every string in both locales. `pytest tests/forms/definitions -q`
...
then a module in `app/forms/features/` fulfils section 6 and `feature_for`
...
4. **An architecture exception**, declared and justified in the plan, never
   silent. The platform has none today; `if key == ...` in `app/forms/` is
...
- `tests/forms/definitions`, `tests/forms/engine`, `tests/forms/features`,
  `tests/forms/adapters`: the compiler, `decide`, the rules, the session, the
  feature modules and the adapter choreography.
- `make lint`: `ruff` and `mypy --strict` over `app/forms/` and
