...
All user-facing strings live in `app/languages/**/*.yml` and are routed through `ml()` (`app/services/utils.py:292`). Embed and button construction is centralized in `app/components/embed.py` and `app/components/buttons.py`; the form screens, modals and cards are drawn by `app/forms/adapters/discord/renderer.py` from the definitions in `app/languages/form/`.
...
### Modals
Drawn by `app/forms/adapters/discord/renderer.py` from the step or section definition.
