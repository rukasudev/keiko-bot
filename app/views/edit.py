...
class EditCommand(discord.ui.View):
    def __init__(self, command_key: str, locale: str):
        self.command_key = command_key
        self.locale = locale
        self.options = self.get_command_options()
        super().__init__()
        self.add_item(
            Select(t("commands.command-event.edit.placeholder", locale=self.locale), self.options)
        )

    def get_command_options(self):
        form_json = parse_json_to_dict(self.command_key, self.locale, "forms.json")
        options = parse_form_cogs_titles(form_json)
        return options
