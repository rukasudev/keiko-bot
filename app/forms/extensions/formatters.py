...
from app.forms.extensions.dates import MONTH_KEYS
...
def _mode_label(value: str, locale: str) -> str:
    return text(f"buttons.summary-card.{value}-label", locale) or value
...
def format_single(value: Any, style: str | None, locale: str) -> str:
    ...
    if value in ("default", "custom"):
        return _mode_label(str(value), locale)
    if style == "boolean":
