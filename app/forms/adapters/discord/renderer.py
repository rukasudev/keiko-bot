...
class ActionButton(discord.ui.Button[Any]):
    ...
    def __init__(self, dispatcher: Dispatcher, custom_id: str, spec: Button, **kw: Any):
        super().__init__(
            label=spec.label,
...
class ChannelPicker(ValuesSelect, discord.ui.ChannelSelect[Any]):
    ...
    def __init__(self, dispatcher: Dispatcher, custom_id: str, spec: Picker, **kw: Any):
        super().__init__(
            custom_id=custom_id,
            ...
            default_values=[
                discord.SelectDefaultValue(
                    id=int(v), type=discord.SelectDefaultValueType.channel
                )
                for v in spec.selected
                if v.isdigit()
            ],
            **kw,
        )
...
class RolePicker(ValuesSelect, discord.ui.RoleSelect[Any]):
    ...
    def __init__(self, dispatcher: Dispatcher, custom_id: str, spec: Picker, **kw: Any):
        super().__init__(
            ...
            default_values=[
                discord.SelectDefaultValue(
                    id=int(v), type=discord.SelectDefaultValueType.role
                )
                for v in spec.selected
                if v.isdigit()
            ],
...
class UserPicker(ValuesSelect, discord.ui.UserSelect[Any]):
    ...
    def __init__(self, dispatcher: Dispatcher, custom_id: str, spec: Picker, **kw: Any):
        super().__init__(
            ...
            default_values=[
                discord.SelectDefaultValue(
                    id=int(v), type=discord.SelectDefaultValueType.user
                )
                for v in spec.selected
                if v.isdigit()
            ],
...
class OptionsPicker(ValuesSelect, discord.ui.Select[Any]):
    ...
    def __init__(
        self, dispatcher: Dispatcher, custom_id: str, spec: OptionSelect, **kw: Any
    ):
        super().__init__(
            custom_id=custom_id,
...
def _picker_max(spec: Picker) -> int:
    return 1 if spec.unique else 25


def _picker_min(spec: Picker) -> int:
    """A single pick may stay empty unless required; a multi pick needs one."""
    return 1 if spec.required or not spec.unique else 0
...
def _select_item(
    ...
    if isinstance(component, Picker):
        ...
        return picker_type(
            ...
            custom_id,
            component,
            min_values=_picker_min(component),
            max_values=_picker_max(component),
        )
...
def _button(dispatcher: Dispatcher, ids: Ids, spec: Button, **kw: Any) -> ActionButton:
    action, _, arg = spec.action.partition(":")
    return ActionButton(dispatcher, ids(action, arg or None), spec, **kw)
