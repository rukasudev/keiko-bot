...
from app.forms.definitions.compiler import (
    CompileError,
    compile_form,
    compile_with_warnings,
)
from app.forms.definitions.json_schema import export
...
def test_the_json_schema_describes_steps_by_kind():
    schema = export()
    assert schema["properties"]["steps"]["type"] == "array"
    kinds = {name for name in schema["$defs"] if name.endswith("Step")}
    assert {"CardStep", "CompositionStep", "TextStep", "ReviewStep"} <= kinds
    assert "en-us" in schema["$defs"]["Text"]["properties"]
